...
{
    uint8_t *write_buffer = (uint8_t *)malloc(sizeof(uint8_t) * (buffer_size + 1));
    if (!write_buffer)
    {
        return ESP_FAIL;
    }
...
void bq25896_init(i2c_port_t _i2c_port)
{
    ...
    Device ID:0x%02x\n \
    Charge Status:%d\n \
    Poweroff voltage:%umV\n \
    Current Limit Pin State:%d\n \
    Current limit:%dmA\n \
    Charge voltage:%dmV\n \
    ...
    Charge state:%d\n",
           ...
           bq25896_get_power_off_voltage(),
           bq25896_get_current_limit_pin_state(),
...
void bq25896_set_precharge_current(uint16_t milliamps)
{
    ...
    value &= 0x0F;
    value |= (((milliamps - BQ25896_PRECHARGE_CURR_MIN) / BQ25896_PRECHARGE_CURR_STEP) << 4);
...
void bq25896_set_charge_target_voltage(uint16_t millivolts)
{
    ...
    value &= 0x03;
    value |= (((millivolts - BQ25896_CHARGE_VOLT_MIN) / BQ25896_CHARGE_VOLT_STEP) << 2);
