...
static void scan_draw_ap_infos(uint8_t ap_index) {
  ...
  lv_obj_add_style(scan_view, style_get_background_main(), LV_PART_MAIN);
  ...
  lv_obj_add_style(ssid_lbl, style_get_font_bigfont(), LV_PART_MAIN);

  lv_obj_t *infos_lbl = lv_label_create(scan_view);
  lv_obj_align(infos_lbl, LV_ALIGN_TOP_LEFT, 5, 30);
  char bssid_str[20] = "";
  wifi_get_bssid_string_from_record(ap_index, bssid_str);
  char phy_str[MAX_PHY_SIZE];
  wifi_get_phy_from_record(ap_index, phy_str);
  lv_label_set_text_fmt(
      infos_lbl,
      "BSSID: %s\nChannel: %d  RSSI: %d\nAuthMode: %s  "
      "PHY:%s\nCypher Pairwise:%s Group:%s",
      bssid_str, wifi_get_one_ap_record(ap_index)->primary,
      wifi_get_one_ap_record(ap_index)->rssi,
      wifi_get_auth_string(wifi_get_one_ap_record(ap_index)->authmode), phy_str,
      wifi_get_cipher_string(wifi_get_one_ap_record(ap_index)->pairwise_cipher),
...
static void scan_draw_deauth(uint8_t ap_index) {
  ...
  lv_label_set_text_fmt(deauth_lbl, "Deauth attack on %s",
  ...
  lv_obj_add_style(return_click_lbl, style_get_font_danger(), LV_PART_MAIN);
...
static void scan_view_draw_ap_list() {
  ...
  for (size_t i = 0; i < wifi_get_all_ap_records()->count; i++) {
    ...
    if (i == current_highlight_item) {
      lv_obj_add_style((lv_obj_t *)wifi_labels[i], style_get_font_highlight(),
                       LV_PART_MAIN);
