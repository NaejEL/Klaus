...
esp_err_t sd_init(spi_host_device_t spi_host, gpio_num_t cs) {
  ...
  esp_err_t ret = esp_vfs_fat_sdspi_mount(MOUNT_POINT, &sd_host, &sd_device,
                                          &mount_config, &card);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to mount filesystem. Error: %s\n",
             esp_err_to_name(ret));
    return ret;
  }
...
bool sd_is_present(void) { return sd_present; }