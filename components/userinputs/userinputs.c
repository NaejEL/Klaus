...
esp_err_t userinputs_init(void)
{
    ...
    userinputs_callbacks = malloc(userinput_callback_capacity * sizeof(userinputs_callback));
    ...
    iot_button_register_cb(key_btn, BUTTON_LONG_PRESS_START, keybtn_clicked, NULL);
    ...
    ESP_RETURN_ON_ERROR(pcnt_new_unit(&unit_config, &pcnt_unit), TAG, "PCNT Init failed");
    ...
    ESP_RETURN_ON_ERROR(pcnt_new_channel(pcnt_unit, &chan_a_config, &pcnt_chan_a), TAG, "Failed to init channel A");
    // Channel B
    ...
    ESP_RETURN_ON_ERROR(pcnt_new_channel(pcnt_unit, &chan_b_config, &pcnt_chan_b), TAG, "Failed to init channel A");
...
void userinputs_register_callback(userinputs_callback callback)
{
    ...
    if (userinput_callback_registrered >= userinput_callback_capacity)
    {
        ...
        userinputs_callbacks = realloc(userinputs_callbacks, userinput_callback_capacity);
    }
