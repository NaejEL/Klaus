...
bool wifi_is_connecting(void) { return is_connecting; }
...
esp_err_t wifi_connect(const char *_ssid, const char *_pass,
                       const char *hostname) {
  ...
  int bits = xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT, pdFALSE,
                                 pdTRUE, 100000 / portTICK_PERIOD_MS);
...
const wifi_ap_records_t *wifi_get_all_ap_records(void) {
  if (scanning) { // Someone get the event before we parse the ap_records, parse
                  // them
    ESP_ERROR_CHECK(
        esp_wifi_scan_get_ap_records(&ap_records.count, ap_records.records));
    scanning = false;
...
void wifi_get_bssid_string_from_record(uint8_t record_index, char *buffer) {
  sprintf(buffer, "%02x:%02x:%02x:%02x:%02x:%02x",
          wifi_get_one_ap_record(record_index)->bssid[0],
          wifi_get_one_ap_record(record_index)->bssid[1],
          wifi_get_one_ap_record(record_index)->bssid[2],
          wifi_get_one_ap_record(record_index)->bssid[3],
          wifi_get_one_ap_record(record_index)->bssid[4],
          wifi_get_one_ap_record(record_index)->bssid[5]);
}
...
void wifi_sta_disconnect(void) { ESP_ERROR_CHECK(esp_wifi_disconnect()); }
...
void wifi_get_ap_mac(uint8_t *mac_ap) { esp_wifi_get_mac(WIFI_IF_AP, mac_ap); }
